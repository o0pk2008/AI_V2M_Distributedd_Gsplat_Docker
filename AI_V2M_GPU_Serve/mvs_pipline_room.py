    ...
    with open(cameras_path, 'w') as file:
        for line in lines:
            if line.startswith("#") or line.strip() == "":
                ...
                # 解析相机参数行
                parts = line.split()
                # 检查相机模型是否为SIMPLE_RADIAL
                if parts[1] == 'SIMPLE_RADIAL':
                    # 将模型类型改为PINHOLE
                    parts[1] = 'PINHOLE'
                    # 提取相机参数
                    f = float(parts[5])  # SIMPLE_RADIAL的焦距
                    cx = float(parts[6])  # 主点x
                    cy = float(parts[7])  # 主点y
                    # 生成新的行，注意保持格式
                    new_line = f"{parts[0]} PINHOLE {parts[2]} {parts[3]} {f} {f} {cx} {cy}\n"
                    file.write(new_line)
                else:
                    # 对于其他相机模型，保留原有行
                    file.write(line)
...
# Example usage:
input_path = 'E:/Python/Glomap/sfm/sparse/0'
output_path = 'E:/Python/Glomap/sfm/sparse/sparse_txt'
outputBin_path = 'E:/Python/Glomap/sfm/sparse'
colmap_path = 'E:/Python/Glomap/sfm'
image_path = 'E:/Python/Glomap/sfm'
